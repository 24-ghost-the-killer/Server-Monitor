...
function renderTree(items, sectionId) {
    ...
    items.forEach(d => {
        ...
        if (!categoryGroups[cat][d.server_name]) categoryGroups[cat][d.server_name] = [];
        categoryGroups[cat][d.server_name].push(d);
    });
    ...
    sortedCategories.forEach(catName => {
        const servers = categoryGroups[catName];
        ...
        html += `
            <div class="category-block" style="margin-bottom: 2px;">
                <div class="tbl-row category-header" onclick="toggleNode('${catId}')">
                    <div class="tbl-col tbl-col--name">
                        <div class="node-name-cell" style="font-weight: 700;">
                            <i data-lucide="chevron-right" size="13" class="chevron ${isCatExpanded ? 'open' : ''}" style="margin-right: 8px;"></i>
                            ${catName.toUpperCase()}
                        </div>
                    </div>
                </div>
                <div class="category-content" style="display: ${isCatExpanded ? 'block' : 'none'}">
                    ...
                    <div class="tbl-body">
                        ${serverNames.map(sName => renderServerGroup(sName, servers[sName], catId)).join('')}
                    </div>
                </div>
            </div>
        `;
...
function renderServerGroup(sName, items, catId) {
    const groupKey = `group:${catId}:${sName}`;
    ...
    const ipAddress = items.length > 0 ? items[0].target_address : "";
    const displayName = sName.includes("(IP:") ? sName : `${sName} (IP: ${ipAddress})`;
    ...
    let html = `
        <div class="tbl-row tbl-row--server" onclick="event.stopPropagation(); toggleNode('${groupKey}')">
            <div class="tbl-col tbl-col--name">
                <div class="node-name-cell">
                    <button class="expand-btn">
                        <i data-lucide="chevron-right" size="11" class="chevron ${isExpanded ? 'open' : ''}"></i>
                    </button>
                    <span class="mono-text node-name-text">${displayName}</span>
                </div>
            </div>
            ...
            <div class="tbl-col tbl-col--detail"><span style="color: hsl(var(--foreground-subtle)); font-size: 11px;">${items.length} units active</span></div>
        </div>
    `;
    ...
    if (isExpanded) {
        items.sort((a, b) => (a.check_type || "").localeCompare(b.check_type || "")).forEach(check => {
            ...
            html += `
                <div class="tbl-row tbl-row--service tbl-row--indent1">
                    <div class="tbl-col tbl-col--name">
                        <div class="tree-indent-svc">
                            <i data-lucide="${getIcon(check.check_type || "")}" size="12" style="color: hsl(var(--foreground-subtle))"></i>
                            <span class="mono-text" style="color: hsl(var(--foreground-muted)); font-size: 11px;">${check.check_type}</span>
                        </div>
                    </div>
                    ...
                    <div class="tbl-col tbl-col--detail">
                        <span style="color: ${isSmartVerified ? 'hsl(var(--status-online-bright))' : 'hsl(var(--foreground-subtle))'}; font-size: 11px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                            ${check.message}
                        </span>
                    </div>
                </div>
...
function getIcon(type) {
    ...
    if (type.includes("22")) return "terminal";
